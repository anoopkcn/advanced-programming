...
class Matrix : MatrixExpr<Matrix<num>> {
 ...
 public:
  ...
  auto begin() noexcept { return elem.get(); }

  auto end() noexcept { return elem.get() + n * cols; }

  const auto begin() const noexcept { return elem.get(); }

  const auto end() const noexcept { return elem.get() + n * cols; }

  auto get_row() const noexcept { return n; }

  auto get_col() const noexcept { return cols; }

  Matrix<num>& operator+=(const Matrix<num>& rhs){
    /* std::cout << "+=\n"; */
    for(auto i=0; i <n*cols; ++i )
      elem[i]+=rhs[i];
    return *this;
...
template <typename num>
Matrix<num> operator+(Matrix<num>&& lhs, const Matrix<num>&rhs)
{
  /* std::cout << "&&\n"; */
  const std::size_t rows = lhs.get_row();
  const std::size_t cols = lhs.get_col();
...
auto t2 = std::chrono::high_resolution_clock::now();
 ...
 std::cout << "time in ms : " << t_f.count() << "\n";
  /* std::cout << rh[1] << " " << *(rh.end()-1) << std::endl; */
  /* std::cout << m1[1] << " " << m1[99] << std::endl; */
  std::cout << "done\n";
  return 0;
}
